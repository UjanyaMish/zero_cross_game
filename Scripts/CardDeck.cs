...
public partial class CardDeck : Node2D
{
	...
	private AudioStreamPlayer deck_sound;
	...
	public override void _Ready()
	{
		...
		deck_sound = new AudioStreamPlayer();
		AddChild(deck_sound);
		AudioStream audioStream = ResourceLoader.Load<AudioStream>("res://sounds/deck_click_sound.mp3");
		deck_sound.Stream = audioStream;
	...
	{
		//дабы звук не накладывался друг на друга
		if (deck_sound != null && !deck_sound.Playing) 
		{
			var voloume = (int)config.GetValue("effects", "voloume",0);
			deck_sound.VolumeDb= voloume;
			GD.Print("effects voloume" + voloume);
			deck_sound.Play();
		}
	}
